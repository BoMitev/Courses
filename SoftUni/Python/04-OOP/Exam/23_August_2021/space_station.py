from everland.astronaut.biologist import Biologist
from everland.astronaut.geodesist import Geodesist
from everland.astronaut.meteorologist import Meteorologist
from everland.planet.planet import Planet
from everland.astronaut.astronaut_repository import AstronautRepository
from everland.planet.planet_repository import PlanetRepository
...
class SpaceStation:
    ...
    def _get_astronauts_for_mission(self) -> list:
        available_astronauts = [ast for ast in self.astronaut_repository.astronauts if ast.oxygen > 30]
        if not available_astronauts:
            raise Exception(f"You need at least one astronaut to explore the planet!")

        mission_astronauts = []
        for astronaut in sorted(available_astronauts, key=lambda ast: -ast.oxygen):
